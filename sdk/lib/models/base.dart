...
abstract class NetBase extends Base {
  ...
  static init() async {
    if (Base.host == '') {
      try {
        ...
        if (Base.host != '' &&
            (!Base.host.startsWith('http://') ||
                !Base.host.startsWith('https://'))) {
  ...
  getHttp(String url,
      {bool show400Error = true,
      bool show500Error = true,
      bool ingoreSessionTimeout = false}) async {
    var body;
    try {
      if (url.startsWith('http') == false &&
          dio.options.baseUrl.startsWith('http') == false) {
  ...
  Future<dynamic> putHttp(String url, dynamic data,
      {bool show400Error = true,
      bool show500Error = true,
      bool ingoreSessionTimeout = false}) async {
    try {
  ...
  Future<dynamic> postHttp(String url, dynamic data,
      {bool show400Error = true,
      bool show500Error = true,
      bool ingoreSessionTimeout = false}) async {
    try {
      var body;
      var response =
          await dio.post(url, data: data, options: _prepareOptions());
      body = response.data;
      if (show400Error && body['status'] >= 400 && body['status'] < 500) {
        XController.showMessage(body['msg']);
      }
  ...
  postFile(file, String url, String fileParamName) async {
    ...
    try {
      ...
      if (file is String) {
        formData =
            FormData.fromMap({fileParamName: MultipartFile.fromFileSync(file)});
      } else if (file is File) {
        formData = FormData.fromMap(
            {fileParamName: MultipartFile.fromFileSync(file.path)});
      }
      var response =
  ...
  exceptionFilter(err, {bool ingoreSessionTimeout = false}) {
    if (err is DioError && err.response != null) {
      switch (err.response?.statusCode) {
        case 401:
        case 408:
          if (ingoreSessionTimeout == false &&
              gt.Get.currentRoute != '/login') {
